...
# as we want to handle _MACIG_SSH environment variable.
...
# Aliases must be defined before functions which use them.
...
ls() {
    set -- -F -l "$@"
    _check command ls --color=auto --version && [ -t 1 ] && set -- --color=always "$@" || :
